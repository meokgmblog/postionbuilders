...
# ==========================================
# 3. CHART RENDER ENGINE (ACTIVE CROSSHAIR)
# ==========================================
...
def render_live_chart(tf, count, expiry):
    ...
    if not df.empty:
        ...
        # Candlestick chart trace (Clean hover, keeps crosshair active)
        fig.add_trace(
            go.Candlestick(
                x=df["timestamp"],
                open=df["open"],
                ...
                hovertemplate="%{x|%a %d %b '%y %H:%M}<extra></extra>",
            ),
            row=1,
            col=1,
        )
        ...
        # Active Crosshair Spike Settings
        spike_config = dict(
            showspikes=True,
...
render_live_chart(tf_option, strike_count, expiry_input)import concurrent.futures
from datetime import datetime
import pandas as pd
import plotly.graph_objects as go
from plotly.subplots import make_subplots
