...
def fetch_option_data_parallel(token, option_rows, key_col):
    ...
    for opt_data in results:
        if not opt_data.empty:
            ...
            opt_sub["oi"] = pd.to_numeric(opt_sub["oi"], errors="coerce").fillna(0)
...
def calculate_tradefinder_position_builder(price_df, ce_df, pe_df):
    ...
    # Align option data with Index candle timestamps via left join
    df = pd.merge(clean_price, ce_df, on="timestamp", how="left")
    df = pd.merge(df, pe_df, on="timestamp", how="left").sort_values("timestamp")

    # Forward fill missing OI ticks across market hours
    df["ce_oi"] = df["ce_oi"].ffill().fillna(0)
    df["pe_oi"] = df["pe_oi"].ffill().fillna(0)

    # Calculate OI changes candle-over-candle
    df["ce_oi_diff"] = df["ce_oi"].diff(1).fillna(0)
    df["pe_oi_diff"] = df["pe_oi"].diff(1).fillna(0)

    df["net_oi_change"] = df["pe_oi_diff"] - df["ce_oi_diff"]

    # Safe normalization scaling (prevents division by zero or NaN generation)
    abs_max = max(abs(df["net_oi_change"].min()), abs(df["net_oi_change"].max()))
    max_val = abs_max if abs_max > 0 else 1.0

    df["position_builder_scaled"] = (df["net_oi_change"] / max_val) * 100
    df["position_builder_scaled"] = df["position_builder_scaled"].fillna(0)
...
with chart_placeholder.container():
    try:
        ...
        if "Weekly" in data_source_mode and not opts_df.empty:
            ...
            # Clean strike suffix check for CE and PE contracts
            ce_opts = atm_opts[
                atm_opts[sym_col].astype(str).str.upper().str.contains("CE")
            ]
            pe_opts = atm_opts[
                atm_opts[sym_col].astype(str).str.upper().str.contains("PE")
            ]
